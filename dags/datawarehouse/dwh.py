...
def staging_table():
    ...
    try:
        ...
        ids_in_json = {row["id"] for row in YT_data}
        ...
        logger.error(f"An error has occured while staging the table of {schema}: {e}")
        raise e
...
def core_table():
    ...
    try:
        ...
        logger.error(f"An error occured while updating the {schema}: {e}")
        raise e
    finally:
        if cur and conn:
            close_conn_cursor(conn, cur)
